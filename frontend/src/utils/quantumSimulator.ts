...
export function simulateCircuit(
  ...
  for (const op of sortedOps) {
    switch (op.gate) {
      case 'I': {
        const q = op.targets[0] ?? 0
        if (q < numQubits) {
          state = applySingleQubitGate(state, numQubits, q, MAT_I)
        }
        break
      }
      case 'X': {
        const q = op.targets[0] ?? 0
        if (q < numQubits) {
          state = applySingleQubitGate(state, numQubits, q, MAT_X)
        }
        break
      }
      case 'Y': {
        const q = op.targets[0] ?? 0
        if (q < numQubits) {
          state = applySingleQubitGate(state, numQubits, q, MAT_Y)
        }
        break
      }
      case 'Z': {
        const q = op.targets[0] ?? 0
        if (q < numQubits) {
          state = applySingleQubitGate(state, numQubits, q, MAT_Z)
        }
        break
      }
      case 'H': {
        const q = op.targets[0] ?? 0
        if (q < numQubits) {
          state = applySingleQubitGate(state, numQubits, q, MAT_H)
        }
        break
      }
      case 'S': {
        const q = op.targets[0] ?? 0
        if (q < numQubits) {
          state = applySingleQubitGate(state, numQubits, q, MAT_S)
        }
        break
      }
      case 'T': {
        const q = op.targets[0] ?? 0
        if (q < numQubits) {
          state = applySingleQubitGate(state, numQubits, q, MAT_T)
        }
        break
      }
      case 'CNOT': {
        const [ctrl, trgt] = op.targets
      ...
      case 'M':
        // Measurements in standard projective simulation are evaluated at the end
        break
    }
  }
